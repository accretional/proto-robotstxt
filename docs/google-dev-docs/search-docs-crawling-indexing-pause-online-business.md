<!--
  ...
  Fetched: 2026-07-04T06:23:17Z by tools/google-dev/pull-docs.sh
...
- Update your structured data: If your site uses structured data (for example, Product, Book, Event), make sure to adjust it appropriately (reflecting the current product availability, or changing events to cancelled). If your business has a physical storefront, update Local Business structured data to reflect current opening hours.
...
- If you need to urgently disable the site for 1-2 days, then return an informational error page with a 503 HTTP response status code instead of all content. Make sure to follow the best practices for disabling a site.
...
-  Confirm a 503 HTTP response status code locally by using curl or a similar tool. For example:
```
...
Google generally crawls from the US. If you block the US, Google Search won't be able to access your site at all. We don't recommend that you block an entire region from temporarily accessing your site; instead, we recommend limiting the site functionality for that region.
