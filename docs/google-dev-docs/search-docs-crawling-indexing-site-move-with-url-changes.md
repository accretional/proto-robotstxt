<!--
  ...
  Fetched: 2026-07-04T06:23:26Z by tools/google-dev/pull-docs.sh
...
-  General best practices for site moves. Know what to expect, and how it might affect your users and rankings. If moving from HTTP to HTTPS, review the best practices for HTTPS.
...
Search Console is your friend, especially during a site move. Verify data for each property separately in Search Console. Use the Index Status report for a broad look. Use the Sitemaps report to view how many URLs submitted in a sitemap have been indexed.
...
-  Crawl rate: Set the crawl rate to "Let Googlebot determine" for both the old and new site.
...
-  Manual action for previous spam. If there are any manual actions applied to the new site, address any problems listed there and file a reconsideration request.

-  Removed URLs. Make sure that there aren't any URL removals left over from the previous owner, especially a site-wide URL removal.
...
-  Each new URL should have a self-referencing rel="canonical" <link> tag.
...
-  Implement or turn on the redirects: depending on your redirect strategy, this might mean you push an update to your server configuration files or you update your CMS, likely with custom code.

-   Check the rel="canonical" link annotations and robots meta rules:  Once the redirects are active, ensure that the rel="canonical" link annotations on the new site are using the new URLs. Similarly, if you added noindex robots meta rules to the new site to avoid prematurely indexing the new URLs, make sure to update them.
...
-   Submit a Change of Address in Search Console for the old site.
